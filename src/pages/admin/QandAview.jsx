...
export default function QandAview() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get("http://127.0.0.1:8000/all-questions");
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                const response = await axios.get("http://127.0.0.1:8000/all-categories");
    ...
    const handleAddData = async () => {
        ...
        try {
            const response = await axios.post("http://127.0.0.1:8000/add-question", form);
            if (response.status === 200) { 
                // Refresh data after successful addition
                const updatedResponse = await axios.get("http://127.0.0.1:8000/all-questions");
    ...
    const handleEditSubmit = async () => {
        ...
        try {
            const response = await axios.put(`http://127.0.0.1:8000/update-question/${selectedRow.id}`, form);
    ...
    const handleHideClick = async (id) => {
        try {
            await axios.patch(`http://127.0.0.1:8000/hide-question/${id}`);
